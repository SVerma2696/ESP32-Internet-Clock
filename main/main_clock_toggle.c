...
// Include our new button and analog clock headers
#include "buttons.h"
...
// We expose this so analog_clock.c can draw geometric shapes to our buffer
void draw_pixel(int x, int y, int color) {
    if(x >= 0 && x < 128 && y >= 0 && y < 64) {
        if (color) fb[y / 8][x] |= (1 << (y % 8));
        else fb[y / 8][x] &= ~(1 << (y % 8));
    }
}
...
void draw_text(int x, int y, const char *str, int scale) {
    while (*str) {
        char c = *str;
        if (c >= 32 && c <= 90) {
            int font_idx = c - 32;
...
void app_main(void) {
    ...
    while (1) {
        ...
        bool current_12h = get_12h_mode();
        bool current_analog = get_analog_mode();

        // Only update the display if the second has passed, or if a button was pressed!
        if (timeinfo.tm_sec != last_sec || current_12h != last_12h || current_analog != last_analog) {
            last_sec = timeinfo.tm_sec;
            last_12h = current_12h;
            ...
            if (current_analog) {
                draw_analog_clock(timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
            } else {
                if (current_12h) {
                    int h = timeinfo.tm_hour % 12;
                    if (h == 0) h = 12;
                    snprintf(strftime_buf, sizeof(strftime_buf), "%02d:%02d:%02d", h, timeinfo.tm_min, timeinfo.tm_sec);
                    draw_text(16, 16, strftime_buf, 2);
                    
                    // Show AM/PM indicator centered between time and date
                    draw_text(58, 34, timeinfo.tm_hour >= 12 ? "PM" : "AM", 1);
                } else {
        ...
        // Delay drastically lowered so buttons feel incredibly responsive.
